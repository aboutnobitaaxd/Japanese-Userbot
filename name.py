from Japanese import bot
from Japanese utils import admin_cmd
import Japanese.plugins.sql_helper.fsub_sql as sql
...
async def fs(event):
  ...
  if not await is_admin(event, NOBITA_XD):
  ...
  if args in ("off", "no", "disable"):
    ...
    rip = await check_him(channel, NOBITA_XD)
    if rip is False:
 ...
 if chat_db:
  try:
    ...
    if rip is False:
       ...
       NOBITA_XD = Var.TG_BOT_USER_NAME_BF_HER
       await bot(EditBannedRequest(event.chat_id, event.sender_id, MUTE_RIGHTS))
       response = await bot.inline_query(NOBITA_XD, "NOBITA_XD")
...
@xbot.on(events.InlineQuery(pattern='NOBITA_XD'))
async def (event):
   global rk
   NOBITA_XD = event.builder
   NOBITA_XD = [[Button.inline("Unmute Me 😊", data="NOBITA_XD")]]
   NOBITA_XD = NOBITA_XD.article(title='FORCE SUBSCRIBE', text='fsub', buttons=NOBITA_XD)
...
import re     
@xbot.on(events.callbackquery.CallbackQuery(re.compile(b"NOBITA_XD")))
